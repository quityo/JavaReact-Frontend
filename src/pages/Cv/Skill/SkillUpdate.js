...
export default function SkillUpdate({ cvId, updateCvValues }) {
    ...
    const handleDeleteSkill = (skillId) => {
        skillService.delete(skillId).then((result) => {
            toast.success(result.data.message)
            skillService.getByCvId(cvId).then((result) => {
              setSkills(result.data.data)
            })
            ...
            <Card fluid color={"black"}>
              ...
              <Card.Content>
                <Form onSubmit={formik.handleSubmit}>
                  ...
                  />
                  {formik.errors.name && formik.touched.skillName && (
              ...
              <Table.Body>
                {skills?.map((skill) => (
                  <Table.Row key={skill.skillId}>
                    ...
                    <Table.Cell>
                      <Button color="red" icon="x" circular onClick={() => handleDeleteSkill(skill.skillId)}>
                      </Button>
                    </Table.Cell>
                  </Table.Row>
                ))}
